...
export function AppShell({ user, children }: { user: { name: string; role: string }; children: React.ReactNode }) {
  ...
  return <div className="min-h-dvh">
    ...
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col border-r bg-card px-5 py-8 lg:flex">
      ...
      <div className="mt-auto flex items-center justify-between border-t pt-5"><Link href="/profile" className="flex min-w-0 items-center gap-3"><span className="flex size-10 shrink-0 items-center justify-center rounded-full bg-muted font-semibold">{user.name.slice(0, 1)}</span><span className="truncate text-sm font-medium">{user.name}</span></Link><form action={logout}><Button variant="ghost" size="icon" aria-label="Keluar"><LogOut className="size-4" /></Button></form></div>
    </aside>
    <div className="lg:pl-64"><header className="flex h-20 items-center justify-between px-5 sm:px-8 lg:px-10"><Link href="/dashboard" className="flex items-center gap-2 font-semibold lg:hidden"><House className="size-6 text-primary" />MyKontrakans</Link><span className="hidden text-sm text-muted-foreground lg:block">Rumah yang nyaman, kas yang transparan.</span><div className="flex items-center gap-2"><ThemeToggle /><Link href="/profile" aria-label="Buka profil" className="flex size-11 items-center justify-center rounded-full bg-primary/10 font-semibold text-primary">{user.name.slice(0, 1)}</Link></div></header>
